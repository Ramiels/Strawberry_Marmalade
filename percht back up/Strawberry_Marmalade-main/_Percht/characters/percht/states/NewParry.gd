...
func get_whiffed_block():
	...
	if prev and prev.get("IS_NEW_PARRY"):
		return ( not prev.parried_last and not prev.autoguard and host.combo_count <= 0)
	return false
...
func get_last_action_text()->String:
	if push:
		return ""
	return ("%sf" % data["Melee Parry Timing"].count) if not reblock else ""
...
func start():
	...
	var high_anim = "ParryHigh" if not use_guard_sprites else "ShieldHigh"
	var low_anim = "ParryLow" if not use_guard_sprites else "ShieldLow"
	if host.is_grounded():
...
func is_usable():
	...
	if current and current.get("IS_NEW_PARRY"):
		whiffed_block_last = ( not current.parried and not current.autoguard and host.combo_count <= 0)
	return .is_usable() and host.current_state().state_name != "WhiffInstantCancel" and ((whiffed_block_last if reblock else not whiffed_block_last) or push)
...
func _tick():
	...
	if host.opponent.current_state().get("IS_NEW_PARRY"):
		...
		if not push and not host.opponent.current_state().push:
			stop_early_tick = 4
...
func detect(obj):
	if obj and not obj.is_in_group("Fighter") and obj.hitlag_ticks <= 0:
