...
func set_sensible_default(attempted_dir, emit_signal = true):
	print(attempted_dir)
	print((consider_facing and facing == - 1 and not "E" in attempted_dir))
	var dirs_list = DIRS if ("W" in attempted_dir) or (consider_facing and facing == - 1 and not "E" in attempted_dir) else DIRS_LEFT
...
func get_data():
	...
	if force_neutral_when_invisible and not pressed_button.is_visible_in_tree():
